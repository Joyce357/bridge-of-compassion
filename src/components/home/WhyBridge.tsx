...
export default function WhyBridge() {
  return (
    <section className="section-py bg-brand-sky/40 border-y border-brand-sky/70" aria-labelledby="why-bridge-heading">
      <Container>
        ...
        <div className="text-center max-w-2xl mx-auto mb-7 sm:mb-9">
          ...
          <h2
            ...
            className="text-brand-navy text-2xl sm:text-3xl lg:text-4xl font-extrabold mt-1.5 mb-2.5 text-balance tracking-tight"
          >
        ...
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
          {pillars.map((pillar) => (
            <div
              key={pillar.number}
              className="relative p-5 sm:p-6 rounded-2xl bg-brand-warm-white border border-border-soft
                         hover:border-brand-green/40 hover:shadow-card transition-all duration-200"
            >
              {/* Number watermark */}
              <span className="text-5xl sm:text-6xl font-black text-brand-navy opacity-10 absolute top-4 right-5 leading-none select-none">
                {pillar.number}
              </span>
              {/* Content */}
              <div className="relative z-10">
                <h3 className="text-base sm:text-lg font-bold text-brand-navy mb-1.5">
                  {pillar.title}
                </h3>
